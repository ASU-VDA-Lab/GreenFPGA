...
def greenfpga_func(args):
    ...
    if args.uncertain_off:
        ...
        if args.carbon_intensity is not None:
            ...
            carbon_per_kWh = 651 # Obtained from expected values

        if args.defect_density is not None:
            defect_density = float(args.defect_density)
            #calculating cpa for 1mm2 case
            yield_val = (1+(defect_density*1e4)*(0.01*1e-6)/10)**-10
            cpa_values = (carbon_per_kWh*epa) + gpa + 500 / yield_val 
            cpa_values = cpa_values*0.01
            scaling_factors['defect_den'].loc[10, 'defect_density'] = float(args.defect_density)
            scaling_factors['cpa'].loc[10, 'cpa'] = cpa_values
        else:
            scaling_factors['cpa'].loc[10,'cpa'] = 13.27 #Obtained from expected values 
            scaling_factors['defect_den'].loc[10,'defect_density'] = 0.12 #Obtained from expected values 
    ...
    tot=emb_c+ope_c
    tot10 = emb_c+(ope_c*0.1) #0.1 use Time
    tot20 = emb_c+(ope_c*0.2) #0.2 use Time
    tot30 = emb_c+(ope_c*0.3) #0.3 use Time
    tot40 = emb_c+(ope_c*0.4) #0.4 use Time
    tot50 = emb_c+(ope_c*0.5) #0.5 use Time
    tot60 = emb_c+(ope_c*0.6) #0.6 use Time
    tot70 = emb_c+(ope_c*0.7) #0.7 use Time
    tot80 = emb_c+(ope_c*0.8) #0.8 use Time
    tot90 = emb_c+(ope_c*0.9) #0.9 use Time
    tot100 = emb_c + ope_c
    #print("-------------------------")
    ...
    #return emb_c, ope_c, tot
    return emb_c, ope_c, tot, tot10,tot20,tot30,tot40,tot50,tot60,tot70,tot80,tot90,tot100
